...
class CategoriesTab extends StatefulWidget {
  ...
  const CategoriesTab({super.key, this.onDataChanged});
...
class _CategoriesTabState extends State<CategoriesTab> {
  ...
  Future<void> _loadCategories() async {
    ...
    try {
      // Usar /all para obtener categorías activas e inactivas
      ...
      if (mounted) {
        ...
        _showSnackBar('Error al cargar categorías', Colors.red.shade600,
            Icons.error_rounded);
      }
    }
  }
  ...
  Future<void> _toggleStatus(Category category) async {
    try {
      if (category.state) {
        ...
        _showSnackBar(
            'Categoría desactivada', primaryOrange, Icons.check_circle_rounded);
      } else {
        await CategoryService.restore(category.idCategory!);
        _showSnackBar('Categoría reactivada', Colors.green.shade600,
  ...
  Future<void> _deleteCategory(Category category) async {
    final confirmed = await _showConfirmDialog(
      'Eliminar categoría',
      '¿Está seguro de eliminar "${category.name}"? Esta acción no se puede deshacer.',
    ...
    if (confirmed == true) {
      try {
        ...
        _loadCategories();
        _showSnackBar('Categoría eliminada exitosamente', primaryOrange,
            Icons.check_circle_rounded);
      } catch (e) {
        _showSnackBar('No se pudo eliminar la categoría', Colors.red.shade600,
            Icons.error_rounded);
      }
    }
  }
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: PopupMenuButton<String>(
                          ...
                          onSelected: (value) {
                            if (value == 'edit')
                              _showFormDialog(category: category);
